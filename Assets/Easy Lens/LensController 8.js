...
function spawnEnemiesAroundCamera() {
    // Clear previous
    script.cube3d_spawner.clear();
    enemies = [];

    // Spawn enemies on a horizontal ring around the user at ringRadiusMeters distance
...
function tryShootRayFromCamera() {
...
function setupAimAndInteraction() {
    ...
    updateEvt.bind(function() {
        ...
        // Face-player: rotate each enemy cube to look at the camera (origin looking -Z)
        for (var i = 0; i < enemies.length; i = i + 1) {
            var e = enemies[i];
            if (!e.alive) { continue; }
            ...
            var toCam = new vec3(0 - pos.x, 0 - pos.y, 0 - pos.z);
...
script.createEvent("OnStartEvent").bind(function() {
    ...
    spawnEnemiesAroundCamera();
    setupAimAndInteraction();
});
