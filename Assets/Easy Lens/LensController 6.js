...
//@input Component.ScriptComponent mutant_body_morph_enemy
...
function spawnEnemies() {
    // Spawn stationary 3D Mutant body morph proxies on a ring around player (using screen space positions)
    ...
    for (var i = 0; i < enemyCount; i = i + 1) {
        ...
        // We do not have API to instantiate multiple FullBodyCostume instances.
        // Store screen-space position proxies to represent 3D enemies for ray hits.
        var enemy = {
            name: "Mutant_" + String(i),
            position2D: pos,
            alive: true
        };
        enemies.push(enemy);
...
function tryShootRay() {
    ...
    for (var i = 0; i < enemies.length; i = i + 1) {
        ...
        // Treat each 3D enemy as a billboarded 2D hit proxy in screen space
        var halfW = enemySize * 0.5;
        var halfH = enemySize * 0.5;
        var minPt = new vec2(e.position2D.x - halfW, e.position2D.y - halfH);
        var maxPt = new vec2(e.position2D.x + halfW, e.position2D.y + halfH);
...
script.createEvent("OnStartEvent").bind(function() {
    ...
    enemySize = Math.min(screenSize.x, screenSize.y) * 0.12; // proxy hit size for 3D mutants
    ringRadius = Math.min(screenSize.x, screenSize.y) * 0.35;

    // Ensure Full Body Costume is enabled (no per-instance API available)
    // Multiple enemies are represented via screen-space proxies around the user.
